...
import { getMyProfile, updateMeternalUncleInfo } from '../../../services/UserService';
...
const EditMaternalUncleForm = () => {
    ...
    const getUncleInfo = () => {
        getMyProfile().then(profile => {
            ...
            if (profile.data()?.m_uncleInfo) {
                setUncles(profile.data()?.m_uncleInfo);
            }
        }).catch(err => {
            console.log(err);
        });
    ...
    const submitUncle = () => {
        ...
        // console.log(tmpUncles);return;

        setIsVisible(false);
        updateMeternalUncleInfo(tmpUncles).then(async res => {
            await getUncleInfo();
            setFormState(null);
...
export default EditMaternalUncleForm
